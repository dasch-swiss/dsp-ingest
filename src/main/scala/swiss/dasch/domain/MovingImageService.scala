...
case class MovingImageService(storage: StorageService, config: SipiConfig) {
  ...
  final private class ProcessOutput(val out: String, val err: String)
  ...
  def extractMetadata(file: DerivativeFile, assetRef: AssetRef): Task[MovingImageMetadata] =
    for {
      ...
      metadata <- executeCommand(command).flatMap(parseMetadata)
    } yield metadata

  private def executeCommand(command: String) = {
    val logger = InMemoryProcessLogger()
    for {
